...
import { formatCep, formatCpf, formatCurrency, formatPhone, onlyNumbers } from '../../utils/format';
...
function CustomerDetail() {
    ...
    async function listBillings() {
        try {
            ...
            const response = await axios.get(`/billings/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    return (
        <div className='container'>
            ...
            <div className='container-main'>
                ...
                <main className="customers-detail-main">
                    ...
                    <div className='customer-data'>
                        ...
                        <button className='edit-customer-btn' onClick={() => setOpenEditClientModal(true)}>
                            <img src={editCustomerProfile} alt='editCustomerProfile' />
                            <span>Editar Cliente</span>
                        </button>
                        <button onClick={() => setOpenRegisterBillModal(true)} className='add-bill-btn'>
                        ...
                        <div className='customer-data-main'>
                            <div className='customer-data-top'>
                                ...
                                <div className='telephone'>
                                    <h2>Telefone</h2>
                                    <span>{customer.telephone && formatPhone(customer.telephone)}</span>
                                </div>
                                <div className='cpf'>
                                    <h2>CPF</h2>
                            ...
                            <div className='customer-data-top'>
                                <div className='email'>
                                    <h2>Endereço</h2>
                                    <span className={customer.street ? '' : 'complement-vazio'} >{customer.street ? customer.street : '-'}</span>
                                </div>
                                <div className='telephone'>
                                    <h2>Bairro</h2>
                                    <span className={customer.district ? '' : 'complement-vazio'} >{customer.district ? customer.district : '-'}</span>
                                </div>
                                <div className='cpf'>
                                    <h2>Complemento</h2>
                                    <span className={customer.complement ? '' : 'complement-vazio'} >{customer.complement ? customer.complement : '-'}</span>
                                </div>
                                <div className='cep'>
                                    <h2>CEP</h2>
                                    <span className={customer.cep ? '' : 'complement-vazio'}>{customer.cep ? formatCep(customer.cep) : '-'}</span>
                                </div>
                                <div className='city'>
                                    <h2>Cidade</h2>
                                    <span className={customer.city ? '' : 'complement-vazio'}>{customer.city ? customer.city : '-'}</span>
                                </div>
                                <div className='state'>
                                    <h2>UF</h2>
                                    <span className={customer.state ? '' : 'complement-vazio'}>{customer.state ? customer.state : '-'}</span>
                                </div>
                            </div>
                                ...
                                <thead>
                                    <tr>
                                        <th className='order-detail-bill'>
                                            <img className='order-client-icon' src={orderIcon} alt='Order icon' onClick={() => handleOrderById()} ></img>
                                            ID Cob.
                                        </th>
                                        <th className='order-detail-bill'>
                                            <img className='order-client-icon' src={orderIcon} alt='Order icon' onClick={() => handleOrderByDate()}></img>
                                            Data de venc.
                                        </th>
                                        <th>Valor</th>
                                        <th>Status</th>
                                ...
                                <tbody>
                                    {billings.map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <td className='status'>
                                                <div className={
                                                    (item.status).toLowerCase() === 'pendente' ? 'status pendente' : (item.status).toLowerCase() === 'vencida' ? 'status vencida' : 'status paga'
                                                }>
                                                    {item.status}
                                                </div>
                                            </td>
                                            <td className="bill-description-detail">{item.description}</td>
                                            <td><img className='edit' onClick={() => handleEdit(item)} src={editBlling} alt='editBlling' /></td>
                                            <td><img className='del' onClick={() => handleDelete(item)} src={deleteBlling} alt='deleteBlling' /></td>
            ...
            {openRegisterBillModal && <RegisterBillModal
