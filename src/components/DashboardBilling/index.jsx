    ...
    <div className='container-dashboard-billing'>
      ...
      <section className='billing-top'>
        ...
        <div className='billing-top-btn'>
          ...
          <div className="input-filter">
            <input placeholder='Pesquisar' onInput={(e) => searchBilling(e, billing, setBilling, billingsTotal)}></input>
      ...
      <section className="style-table">
        <table className="dash-billing">
          <thead>
            <tr>
              <th className="dash-billing-order">
                <img src={orderIcon} alt='Order icon' onClick={() => handleOrderByName()}></img>
                Cliente
              </th>
              <th className="dash-billing-order">
                <img src={orderIcon} alt='Order icon' onClick={() => handleOrderByID()}></img>
                ID Cob.
              </th>
              <th>Valor</th>
              <th>Data de venc.</th>
          ...
          {
            billingsFiltradas[0] === 'notFound' && <div className="searchNotFound">
              <img src={searchNotFound} alt='pesquisa nao encontada' />
            </div>
          }
          <tbody>
            {billingsFiltradas[0] !== 'notFound' && billingsFiltradas.map((item) => (
              <tr className='customer-name' key={item.id}>
                ...
                <td onClick={() => handleDetailBilling(item)}>R$ {formatCurrency(item.value)}</td>
                <td onClick={() => handleDetailBilling(item)}>{format(new Date(item.due_date), 'dd/MM/yyyy')}</td>
                <td onClick={() => handleDetailBilling(item)}>
                  <div className={
                ...
                <td onClick={() => handleDetailBilling(item)} className="bill-description">{item.description}</td>
                <td>
                  <div className='column-btn'>
                    <div className='column-btn-icon edit'>
                      <img onClick={() => handleEdit(item)} src={editIcon} alt='Bill icon'></img>
                      Editar
                    </div>
                    <div className='column-btn-icon del'>
                      <img onClick={() => handleDelete(item)} src={deleteIcon} alt='Delete icon'></img>
                      Excluir
                    </div>
                  </div>
                </td>
